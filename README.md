...
Some legacy workflows use [cytominer-database](https://github.com/cytomining/cytominer-database), including the CSV collation utilities described below.
...
Pycytominer supports data generated by [CellProfiler](https://cellprofiler.org/) and includes defaults that match common CellProfiler data structures and naming conventions.
Pycytominer can process appropriately structured CellProfiler outputs directly, but for most workflows we recommend harmonizing CellProfiler outputs with [CytoTable](https://github.com/cytomining/CytoTable) before using Pycytominer: CellProfiler -> CytoTable -> Pycytominer.
For a CellProfiler-specific example, see the [CytoTable CellProfiler-to-Parquet tutorial](https://cytomining.github.io/CytoTable/tutorials/cellprofiler_to_parquet.html).
For a broader overview, see the [CytoTable mise en place guide](https://cytomining.github.io/CytoTable/examples/cytotable_mise_en_place_general_overview.html).
...
### Harmonizing inputs from image analysis tools
...
For example, to resolve potential feature issues in the `normalize()` function, you may need to manually specify the morphological features using the `features` [parameter](https://pycytominer.readthedocs.io/en/latest/pycytominer.html#pycytominer.normalize.normalize).
...
When parquet-backed CytoTable or CytoDataframe inputs include OME-Arrow image payload columns, pycytominer still supports numeric `Image_*` measurements when you request image features. At the same time, inferred `Image_*` features are limited to numeric columns, and `normalize()` only accepts numeric feature columns. Non-normalized image payload columns are preserved in normalized outputs rather than being transformed. This lets pycytominer analyze image-level measurements without trying to normalize nested image payload data stored alongside them.

If you are using Pycytominer with these tools or data systems, please file [an issue](https://github.com/cytomining/pycytominer/issues) to reach out.
